...
echo ""
echo "    Нажми Enter чтобы запустить opencode"
echo "    (или закрой Termux и открой заново)"
echo "  ══════════════════════════════════════"
read -r
...
if command -v opencode &>/dev/null; then
  ...
  echo ""
  echo "  Не удалось запустить opencode автоматически."
  echo "  Закрой Termux и открой заново, потом напиши: opencode"
fi
